...
class Attendee {
    ...
    constructor(peerSettings) {
        log.debug(`The PeerJS settings: ${JSON.stringify(peerSettings)}`);
        this._peer = new Peer(peerSettings);
        log.info(`Using the PeerJS server: ${peerSettings?.host || "default"})`);

        const attendeeSelf = this;
        this._peer.on("open", peerId => {
            log.info(`Connected to the PeerJS server. (peerId = ${peerId})`);
...
export class Host extends Attendee {
    ...
    constructor(peerSettings) {
        ...
        this._peer.on("connection", guest => {
            log.info(`A guest peer connected: ${JSON.stringify(guest)}`);
    ...
    kick(target) {
        ...
        log.warn("The guest does not exist.");
    }
    ...
    constructor(hostPeerId, peerSettings) {
        ...
        this.on("open", () => {
            ...
            guestSelf._hostConnection.on("open", () => {
                log.info(`Connected to the host. (peerId = ${hostPeerId})`);
                ...
                guestSelf._hostConnection.on("data", data => {
                    log.debug(`Received data from the host: ${JSON.stringifydata}`);
